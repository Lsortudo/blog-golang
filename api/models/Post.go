...
func (p *Post) Validate() error {
	if p.Title == "" {
		return errors.New("Insira um titulo valido")
	}
	if p.Content == "" {
		return errors.New("Insira um conteudo valido")
	}
	if p.AuthorID < 1 {
		return errors.New("Insira um autor valido")
	}
	return nil
}
...
func (p *Post) FindAllPosts(db *gorm.DB) (*[]Post, error) {
	...
	if len(posts) > 0 {
		//for i := range posts { simplificando talvez
		for i, _ := range posts {
...
func (p *Post) DeletePost(db *gorm.DB, pid uint64, uid uint32) (int64, error) {
	...
	if db.Error != nil {
		if gorm.IsRecordNotFoundError(db.Error) {
			return 0, errors.New("Post nao encontrado")
		}
		return 0, db.Error
	}
